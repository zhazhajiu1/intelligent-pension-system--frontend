      ...
      <el-main class="left">
        <div class="video-container">
          ...
          <el-card class="box-card">
            <el-button @click="getSRC">获取视频源</el-button>
            <el-button @click="startDraw">开始绘制</el-button>
            <el-button @click="clearAll">清除所有绘制</el-button>
            <el-button @click="sendPoints">发送坐标</el-button>
            <el-button @click="getSRC4">获取新视频</el-button>
          </el-card>
        </div>
      </el-main>
    </el-container>
...
export default {
  data() {
    return {
      ...
      drawCanvas: null,
      drawCtx: null,
      isVideoFetching: true // 添加布尔变量控制视频源获取状态
    };
  },

  methods: {
    async getSRC() {
      if (!this.isVideoFetching) return; // 如果 isVideoFetching 为 false，则不执行获取
      try {
      ...
      if (this.points.length === 4) {
        ...
        try {
          ...
          if (response.code === 20000) {
            ...
            this.isVideoFetching = false; // 成功发送坐标后停止获取视频源
          } else {
